...
struct SendPost: View {
    ...
    func createPost() {
        let addPost = Post(id: UUID().uuidString, title: self.title, content: self.content, user: (session.session?.email)!, comments: [], created_at: Timestamp(date: Date()))
        do {
            try db.collection("basliklar").document(addPost.id).setData(from: addPost)
